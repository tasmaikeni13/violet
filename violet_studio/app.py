...
class VioletStudioApp(tk.Tk):
    ...
    def _update_plaintext_count(self) -> None:
        ...
        count = sum(character in ALPHABET for character in content.upper())
    ...
    def _start_worker(self, mode: str) -> None:
        ...
        def worker() -> None:
            ...
            try:
                ...
                result = machine.encrypt(payload) if mode == "encrypt" else machine.decrypt(payload)
        ...
        try:
            while True:
                ...
                if mode == "error":
                    ...
                    messagebox.showerror("Operation failed", message, parent=self)
                    continue
    ...
    def _update_live_analysis(self) -> None:
        ...
        plaintext = "".join(
            character
            for character in self.live_input_text.get("1.0", "end-1c").upper()
        ...
        for char in plaintext:
            ...
            if not all(int(permutation[int(permutation[index])]) == identity[index] for index in range(26)):
                non_reciprocal += 1
            output_char = machine.encrypt_char(char)
            if output_char == char:
                fixed_steps += 1
